...
TILE_SIZE = 40                               # her bir karenin ne kadar geniş olacağı pixel cinsinden
...
class Cell:                                 # o karelere hücre diyoruz ve onun sınıfı burada ve değerlerini giriyoruz
    ...
    def draw_current_cell(self):            # bu ve aşaıdaki draw fonksiyonları tamamen boyama işlemi bu harityaı oluşturan hücre için
        ...
        pygame.draw.rect(sc, pygame.Color(122,86,236), (x + 2 + int(TILE/4), y + 2 + int(TILE/4), (int(TILE/2) - 1), (int(TILE/2) - 1)))

    def draw_start_cell(self):              # bu başlangıç hücresini boyamak için 
        x, y = self.x * TILE, self.y * TILE
        pygame.draw.rect(sc, pygame.Color(90,156,255), (x + 2 + int(TILE/4), y + 2 + int(TILE/4), (int(TILE/2) - 1), (int(TILE/2) - 1)))

    def draw_end_cell(self):                # bu bitiş hücresini boyamak için 
        x, y = self.x * TILE, self.y * TILE
        pygame.draw.rect(sc, pygame.Color(5,74,150), (x + 2 + int(TILE/4), y + 2 + int(TILE/4), (int(TILE/2) - 1), (int(TILE/2) - 1)))

    def draw_travelled_cell(self):          # bu cevabı gezen hücreyi boyamak için 
        x, y = self.x * TILE, self.y * TILE
        pygame.draw.rect(sc, pygame.Color(238,162,104), (x + 2 + int(TILE/4), y + 2 + int(TILE/4), (int(TILE/2) - 1), (int(TILE/2) - 1)))

    def draw_past_cell(self):               # bu cevabı gezen hücrenin gezdiği yerleri boyamak için 
        x, y = self.x * TILE, self.y * TILE
        pygame.draw.rect(sc, pygame.Color(248,230,197), (x + 2 + int(TILE/4), y + 2 + int(TILE/4), (int(TILE/2) - 1), (int(TILE/2) - 1)))

    def draw_way_cell(self):                # bu en kısa yolu boyamak için 
        x, y = self.x * TILE, self.y * TILE
        pygame.draw.rect(sc, pygame.Color(241,194,105), (x + 2 + int(TILE/4), y + 2 + int(TILE/4),  (int(TILE/2) - 1), (int(TILE/2) - 1)))
    ...
    def draw(self):                         # burada duvarlar boyanıyor yani başlanıgçta ızgara daha sonra harita oluyor
        ...
        if self.walls['top']:
            pygame.draw. line(sc, pygame.Color(132,50,27), ((x), y), (x + TILE, y), 1)
        if self.walls['right']:
            pygame.draw.line(sc, pygame. Color(132,50,27), ((x + TILE ), y), ((x + TILE ), y + TILE), 1)
        if self.walls['bottom']:
            pygame.draw.line(sc, pygame.Color (132,50,27), (x + TILE, (y + TILE)), (x , (y + TILE)), 1) 
        if self.walls['left']:
            pygame.draw. line(sc, pygame.Color (132,50,27), ((x), y + TILE), ((x ), y) , 1)
    ...
    def check_neighbors(self):              # burada komşu kontrolü yapılıyor 
        ...
        if top and not top.visited and not top.wall:         # eğer üstümüzde komşu varsa ve daha önce ziyaret edilmemişse komşu septimize atıyoruz.
            neighbors.append(top)
        if right and not right.visited and not right.wall:     # aynı şeyi 3 yön için yapıyoruz. burası haritayı oluştururken daha önce geçip geçmediğimizi kontrol için
            neighbors.append(right)
        if bottom and not bottom.visited and not bottom.wall:   # bu koda çok benzeri aşağıda var o da cevabı aramak için karıştırma arada fark var
            neighbors.append(bottom)
        if left and not left.visited and not left.wall:
            neighbors.append(left)
        selected = choice(neighbors)
        if selected == left :
            make_wall_true(self.x, self.y - 1)
            make_wall_true(self.x, self.y + 1)
        if selected == right:
            make_wall_true(self.x, self.y - 1)
            make_wall_true(self.x, self.y + 1)
        if selected == top:
             make_wall_true(self.x + 1, self.y)
             make_wall_true(self.x - 1, self.y)
        if selected == bottom:
             make_wall_true(self.x + 1, self.y)
             make_wall_true(self.x - 1, self.y)
        return selected if neighbors else False
...
current_cell = grid_cells[TILE_SIZE + 1]                # haritayı oluşturacak olan hücremizi dünyaya salıyoruz
check_cell = grid_cells[TILE_SIZE + 1]                  # haritayı oluşturacak olan hücremiz bütün oluşturmayı tamamlanınca başladığı yere geri gelicek onun kontrolü için bunu oluşturdum
...
def make_wall_true(x, y):             # burada hücrenin varlığı kontrolü yapılıyor bize verilen kordinatlarda hücre olup olmadığı sorglanıyor
        find_index = lambda x, y: x + y * cols
        if x <= 0 or x >= cols - 1 or y <= 0 or y >= rows - 1:
            return False
        selected_cell = grid_cells[find_index(x, y)]
        selected_cell.wall = True
...
while True:                                 # işte sürekli döngüye giren çalışan yer burası
    ...
    if current_cell == check_cell:          # eve geldiğinde ise bizim seyehatçiyi yola salıyorum
            ...
            if last_piece == 1:             # eğer bitişi bulduysa geldiğin yoldan geri gel emmi diyorum 
                ...
                travelled_places = travelled_places + 1
                ...
                print("geçen süre" + str(total_time))
                print("gezilen yerler " + str(travelled_places))
                last_piece = 1
