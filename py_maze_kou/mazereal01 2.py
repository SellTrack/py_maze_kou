...
RES = WIDTH, HEIGHT = 100, 100              #çözünürlük
...
class Cell:                                 # o karelere hücre diyoruz ve onun sınıfı burada ve değerlerini giriyoruz
    ...
    def draw(self):                         # burada duvarlar boyanıyor yani başlanıgçta ızgara daha sonra harita oluyor
        ...
        if self.walls['top']:
            pygame.draw. line(sc, pygame.Color(132,50,27), (x, (y )), (x + TILE, (y )), int(TILE/2))
        if self.walls['right']:
            pygame.draw.line(sc, pygame. Color(132,50,27), ((x + TILE ), y), ((x + TILE ), y + TILE), int(TILE/2))
        if self.walls['bottom']:
            pygame.draw.line(sc, pygame.Color (132,50,27), (x + TILE, (y + TILE)), (x , (y + TILE)), int(TILE/2)) 
        if self.walls['left']:
            pygame.draw. line(sc, pygame.Color (132,50,27), ((x), y + TILE), ((x ), y) , int(TILE/2))
